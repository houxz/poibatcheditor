...
import cn.emg.poibatcheditor.dao.WayModelDao;
import cn.emg.poibatcheditor.pojo.AdminCodeModel;
import cn.emg.poibatcheditor.pojo.EmployeeModel;

@Controller
@RequestMapping("/way.web")
public class WayCtrl extends BaseCtrl {

	private static final Logger logger = LoggerFactory.getLogger(WayCtrl.class);
	
	@Autowired
	private WayModelDao wayModelDao;
	...
	private ModelAndView controller(Model model, HttpSession session, HttpServletRequest request,
		...
		try {
			switch (action) {
			case getWay:
				result = getWay(module);
				break;
			case getWayByAdminCodes:
	...
	private ResultModel getWay(RequestModule module) {
		logger.debug("START");
		ResultModel result = new ResultModel();
		try {
			...
			List<Map<String, Object>> pois = wayModelDao.select(columns, code, size, (number-1)*size);
			Integer total = wayModelDao.count(columns, code);
			result.setResult(1);
			result.setRows(pois);
	...
	private ResultModel getWayByAdminCodes(RequestModule module) {
		...
		try {
			...
			List<Map<String, Object>> pois = wayModelDao.selectByOwners(columns, owners, size, (number-1)*size);
			Integer total = wayModelDao.countByOwners(columns, owners);
			result.setResult(1);
			result.setRows(pois);
			result.setTotal(total);
		} catch (Exception e) {
